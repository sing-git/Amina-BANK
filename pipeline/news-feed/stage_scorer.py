...
def load_articles_from_signals(company_id: str):
    """Load articles from kyc_drift_signals.json for a specific company.
    Maps full_text -> clean_text and source -> source_feed to match scorer expectations.
    """
    ...
    for sig in company.get("signals", []):
        articles.append({
            ...
            "clean_text": sig.get("full_text", ""),
            "source_feed": sig.get("source", "rss"),
            "companies": [],  # no NER entities in signals file; fuzzy match still works on title
        })
    return articles
...
def entity_fuzzy_score(article: dict, kyc: dict) -> float:
    ...
    Falls back to title + summary fuzzy match when no NER companies are present
    (e.g. when loading from kyc_drift_signals.json which has no NER data).
    """
    targets = [
        kyc["legal_name"],
    ...
    # Fallback: match KYC name directly against title and summary
    ...
    for text in [title, summary]:
        if not text:
            continue
        for target in targets:
            ratio = fuzz.token_set_ratio(text, target) / 100.0
            best = max(best, ratio)
    return round(best, 4)
...
def main():
    ...
    parser.add_argument("--from-signals", action="store_true",
                        help="Read from kyc_drift_signals.json instead of selected_articles.json")
    ...
    if args.from_signals:
        articles = load_articles_from_signals(args.company_id)
        print(f"Loaded {len(articles)} signals from kyc_drift_signals.json")
    else:
